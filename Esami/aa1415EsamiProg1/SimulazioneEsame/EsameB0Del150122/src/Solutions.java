...
public class Solutions {
    ...
    public static boolean exUno(int[][] m){
        boolean controlloElemento = false;
        boolean controlloRighe = true;
        //controllo grandezza del vettore
        if( m != null && m.length > 1 ){
            for( int i = 1; controlloRighe && i< m.length; i++){

                controlloElemento = false;
                for(int j = 0; !controlloElemento && j<m[i].length; j++){
                    if (m[i][j] > m[i-1][j] ){
                        controlloElemento = true;
                    }
                }

                if(!controlloElemento){
                    controlloRighe = false;
                }
            }
        }else{
            controlloRighe = false;
        }
        return controlloRighe;
    }
    ...
    public static int[] exDue(int[] vet){
        exDue(vet,0, vet.length);
        return vet;
    }
    private static void exDue(int[] vet,int i, int lung){
        if(i + 2  == lung ){
