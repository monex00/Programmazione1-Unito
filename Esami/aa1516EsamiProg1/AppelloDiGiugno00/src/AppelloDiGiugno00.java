 ...
 */
    ...
    public static int exUno(int[][] m, int[] a){
        int rmin = -1;
        int rmax = -1;
        int result = 0;
        boolean trovatoMin = false;
        int cont = 0;
        boolean trovatoMax = false;
        if(a.length <= m[0].length) {

            for (int i = 0; !trovatoMin && i < m.length; i++) {
            ...
            cont = 0;
            for (int i = m.length - 1; !trovatoMax && i >= 0; i--) {
                for (int j = 0; j < a.length; j++) {
